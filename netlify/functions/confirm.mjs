// Double-Opt-In Bestätigung (Aufruf aus der Bestätigungs-Mail).
...
export default async (req) => {
  ...
  const email = verifyEmailToken(token, 'confirm');
  if (!email) return page('Link ungültig', 'Dieser Bestätigungslink ist ungültig oder abgelaufen.');
  const k = key(email);
  const rec = (await getItem('subscribers', k)) || { id: k, email, created: Date.now() };
  rec.status = 'confirmed';
  rec.confirmedAt = Date.now();
  await putItem('subscribers', k, rec);
  return page('Anmeldung bestätigt ✓', 'Vielen Dank! Sie erhalten den KI-Lab Newsletter ab der nächsten Ausgabe.');
};
